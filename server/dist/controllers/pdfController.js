...
function drawSummary(doc, income, expenses, balance) {
    const gap = 12;
...
const ROW_H = 22;
const HEADER_H = 24;
function drawTable(doc, title, rows, accent, total) {
    ensureSpace(doc, 90);
    ...
    doc
        ...
        .fontSize(9)
        .fillColor(MUTED)
        .text(`${rows.length} ${rows.length === 1 ? "entry" : "entries"}`, MARGIN, doc.y - 13, { width: CONTENT_WIDTH, align: "right" });
    doc.moveDown(0.6);
    if (rows.length === 0) {
        doc
